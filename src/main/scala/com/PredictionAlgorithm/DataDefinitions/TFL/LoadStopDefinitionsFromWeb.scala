...
object LoadStopDefinitionsFromWeb extends LoadResource{
  ...
  private lazy val stopList:Set[String] = TFLDefinitions.StopToPointSequenceMap.keySet.map {case(_,_,x) => x}
  private def tflURL(stopCode:String):String = "http://countdown.api.tfl.gov.uk/interfaces/ura/instant_V1?StopCode1=" + stopCode + "&ReturnList=StopPointName,StopPointType,Towards,Bearing,StopPointIndicator,StopPointState,Latitude,Longitude"
    ...
    stopList.foreach{ x=>
        val s = Source.fromURL(tflURL(x))
         s.getLines.drop(1).foreach(line => {
           ...
           stopDefinitionMap += (x -> (split(0),split(1),split(2),split(3).toInt, split(4), split(5).toInt,split(6).toDouble,split(7).toDouble))
         }
         )
    }
  ...
  private def persist: Unit = {
    ...
    pw.close
    println("stop definitions persisted to file")
  }
